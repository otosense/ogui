...
const DataTypeOne = (props: IProps) => {
	...
	useEffect(() => {
		...
		if (chart) {
			...
			const yAxisData = updatedSeries
				?.flat()
				.map((series: ISample) => series.value);
			const xAxisTs = updatedSeries
				?.flat()
				.map((series: ISample) => series.time);
    ...
    */
	...
	return (
		<div className="chartParent">
			...
			/>
			<button onClick={handlePan} className="loadMoreButton">
				Load More
			</button>
		</div>
	);
