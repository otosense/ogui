...
function DataTable (props: IDataTableProps): JSX.Element {
  ...
  const captureNewData = (xs: string | any[]): void => {
    setFlatRowData(xs.length > 0 ? xs : dataCopy)
  }

  const togglePatternSearchButton = (): void => {
                    ...
                    <MaterialReactTable
                        ...
                        muiTableBodyRowDragHandleProps={({ table }) => ({ // Row drag handler
                          onDragEnd: () => {
                            ...
                            if ((hoveredRow != null) && (draggingRow != null) && flatRowData) {
                              flatRowData?.splice(
...
function CustomInfoButton (): JSX.Element {
  return <Box>
