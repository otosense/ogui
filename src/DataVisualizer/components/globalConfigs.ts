...
import { IProps, ISrcChannel, IZoomRange } from "./Utilities/interfaces";
...
function defaultZoomBehavior(this: any) {
	const chart = this;
	const resetZoomButton = chart?.renderer
		.button("Reset Zoom", null, null, function () {
...
async function implicitChannelMapping(
	...
	const promises = src_channels.map(
		async (eachChannel: { channel: string }) => {
	...
	try {
		...
		responses.map((response) => {
			...
			const existingChannelIndex = data.findIndex(
				(item: { channel: string }) => item.channel === response.channel
			);
...
async function explicitChannelMapping(
	src_channels: ISrcChannel[],
	start: number,
	newStart: number,
	data: any[],
	...
	try {
        ...
        */
		...
		responses?.forEach((response) => {
			...
			if (existingChannelIndex !== -1) {
				data[existingChannelIndex].data = [
					...data[existingChannelIndex]?.data,
					...response.data,
				];
			} else {
...
function settingZoomInGlobalStore(
	combineZoom: boolean,
	e: IZoomRange,
	props: IProps
) {
	if (combineZoom === undefined ? true : combineZoom) {
		// handling Whether user opted for combineZoom
...
function updatingZoomFromGlobalStore(
	chartRef: any,
	zoomLevel: IZoomRange | null,
	combineZoom: boolean
) {
