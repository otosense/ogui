...
function Load (props: ILoadProps): JSX.Element {
  ...
  const handleDagSubmit = async (event: { preventDefault: () => void }): Promise<void> => {
    // Creating the Payload which backend needs
    event.preventDefault()

    if (loadSavedDag != null) {
      ...
      if (isObject(val)) {
        ...
        if (isFunction(result?.then)) {
          // Check if the result of the function is a promise
          result.then((dataArray: any) => {
            props.onDataUploaded?.(dataArray)
          })
        } else {
          props.onDataUploaded?.(result)
        }
      }
    }
    ;onClose?.()
  ...
  const loadDagHandler = async (e: { target: { value: string } }): Promise<void> => {
